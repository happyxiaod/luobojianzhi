...
<script>
    ...
    var SEL_PATH = "/luoboNew/manage.php/Member/pageSchool";
    var ACT_PATH = "/luoboNew/manage.php/Member/pageSchool";
</script>	
</head>
<body>
		...
		<div class="row fragbox">
			<div class="col-md-11">
				<table class="table text-center mtop1 table-bordered">
					...
					<tr>
						<th class="text-center">学校机构名称</th>
					...
					<?php if(is_array($userSchoolList)): $i = 0; $__LIST__ = $userSchoolList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo["schoolemail"]); ?></td>
							<td>
								<input type="hidden" value="<?php echo ($vo["id"]); ?>" name="id"/>
								<button class="btn btn-danger btn-sm" gt-btn-click="del-member-school">删除</button>
							</td>
...
<script>
	jQuery(document).ready(function($) {
		bindClick();
		$("#navMemberSchool").addClass("nav-two-active"); //更改导航样式
	});
</script>