...
<script>
    ...
    var SEL_PATH = "/index.php/Manage/Setting/pageRegionProvince";
    var ACT_PATH = "/index.php/Manage/Setting/pageRegionProvince";
</script>
</head>
<body>
                    ...
                    <table class="table text-center mtop1">
                        ...
                        <?php if(is_array($provinceList)): $i = 0; $__LIST__ = $provinceList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    ...
                                    <a href="/index.php/Manage/Setting/pageSaveProvince?id=<?php echo ($vo["id"]); ?>" class="btn btn-primary btn-sm">修改</a>
                                </td>
...
<script>
    jQuery(document).ready(function($) {
        ...
        //删除点击事件
        $(".delProvince").on('click', '', function (event) {
            event.preventDefault();
            delProvince($(this));
        });
    });
    ...
    //删除省
    function delProvince(btn) {
        id = btn.siblings("input[name='id']").val();
        jQuery.ajax({
            url: CON_PATH + '/delProvince',
            type: 'POST',
            dataType: '',
            data: {'id': id},
