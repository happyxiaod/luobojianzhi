...
<script>
    ...
    var SEL_PATH = "/luoboNew/manage.php/Audit/pageSeeker";
    var ACT_PATH = "/luoboNew/manage.php/Audit/pageSeeker";
</script>	
</head>
<body>
			...
			<div class="col-md-10">
				<table class="table text-center mtop1 table-hover">
					...
					<?php if(is_array($userSeekerList)): $i = 0; $__LIST__ = $userSeekerList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							<td><input type="checkbox" name="check" value="<?php echo ($vo["id"]); ?>" ><input type="hidden" value="<?php echo ($vo["id"]); ?>" > </td>
							<td style="line-height: 30px;"><?php echo ($vo["realname"]); ?></td>
							<td style="line-height: 30px;"><?php echo ($vo["phone"]); ?></td>
							<td style="line-height: 30px;"><?php echo ($vo["school"]); ?></td>
							<td style="line-height: 30px;"><?php echo ($vo["createtime"]); ?></td>
							<td>
								<input type="hidden" value="">
								<?php if(!empty($vo['idobverse'])){ echo('
									<button class="audit-thumbnail" gt-btn-click="audit-thumbnail"
											popup-src="/luoboNew/Uploads/'.$vo['idobverse'].'">证件正面
									</button>
									'); } ?>
								<?php if(!empty($vo['idreverse'])){ echo('
									<button class="audit-thumbnail" gt-btn-click="audit-thumbnail"
											popup-src="/luoboNew/Uploads/'.$vo['idreverse'].'">证件反面
									</button>
									'); } ?>
							</td>
							<td>
							...
							<td>
								<a href="/luoboNew/manage.php/Audit/pageSeekerInfo?id=<?php echo ($vo["id"]); ?>" target="_blank" class="btn btn-primary btn-sm">查看详情</a>
							</td>
...
jQuery(document).ready(function($) {
		...
		var value='';
		var boxs = $("input[name='check']");
		$("#selectPassAudit").click(function(){
			var ids=[];
			for (var i=0;i<boxs.length;i++ ){
		     if(boxs[i].checked){ //判断复选框是否选中
		...
		$("#selectRefuseAudit").click(function(){
			var ids=[];
			for (var i=0;i<boxs.length;i++ ){
		     if(boxs[i].checked){ //判断复选框是否选中
		     	ids.push(boxs[i].value);
