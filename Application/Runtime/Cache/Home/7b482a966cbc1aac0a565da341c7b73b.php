...
<script>
    ...
    var SEL_PATH = "/index.php/Home/Company/pageEditJob?id=1528";
    var ACT_PATH = "/index.php/Home/Company/pageEditJob";
</script>
...
<script type="text/javascript">
    ...
    <!-- 主体内容 -->
        <div id="person">
        <!-- 个人中心start input 加上class=error 错误样式 -->
        <div class="per_t">
            <div class="per_nav">
    ...
    <ul class="per_nav">
            ...
            <div class="per_r_dal">
                ...
                <div class="per_cont" gt-model="job">
                    <div>
                    <input type="hidden" gt-isparam="id" value="<?php echo ($job["id"]); ?>"/>
                        <span class="label_text">兼职标题</span>
                        <input type="hidden" gt-islabel="兼职标题"/>
                        <input type="text" class="" gt-isparam="title" value="<?php echo ($job["title"]); ?>" />
                      <!--   <input type="text" class="error" gt-isparam="title" value="<?php echo ($job["title"]); ?>" /> -->
                    </div>
                    ...
                    <div>
                        ...
                        <div class="input-group spinner">
                            <select name="level1" gt-isparam="level1"  style="width:100%">
                                <option value="<?php echo $job['level1']?$job['level1']:'选择分类';?>"><?php echo $job['level1name']?$job['level1name']:'选择分类';?></option>
                                <?php if(is_array($level1list)): $i = 0; $__LIST__ = $level1list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["name"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <div class="input-group spinner">
                        <select name="level2" gt-isparam="level2" id="level2ListBox" style="width:100%">
                                <option value="<?php echo $job['level2']?$job['level2']:'选择职位';?>"><?php echo $job['level2name']?$job['level2name']:'选择职位';?></option>
                        </select>
                    ...
                    <div class="add">
                        ...
                        <div class="input-group spinner">             
                            <select name="province" gt-isparam="province"  style="width:100%">
                                    <!-- <option value="<?php echo $job['province']?$job['province']:'请选择省';?>"><?php echo $job['province']?$job['province']:'请选择省';?></option> -->
                                    <option value="<?php echo ($job["province"]); ?>"><?php echo ($job["province"]); ?></option>
                                    <?php if(is_array($provinceList)): $i = 0; $__LIST__ = $provinceList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["name"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <div class="input-group spinner">                         
                            <select name="city" gt-isparam="city" id="cityList" class="select-city" style="width:100%">
                               <option value="<?php echo $job['city']?$job['city']:'选择城市';?>"><?php echo $job['city']?$job['city']:'选择城市';?></option>
                            </select>
                        </div>
                        <div class="input-group spinner">
                            <select name="" gt-isparam="county" id="countyList" style="width:100%">
                               <option value="<?php echo $job['county']?$job['county']:'选择地区';?>"><?php echo $job['county']?$job['county']:'选择地区';?></option>
                            </select>
                        </div> 
                        <input type="hidden" gt-islabel="详细地址"/>                         
                        <input type="text" name="" gt-isparam="address" placeholder="请输入详细地址" value="<?php echo ($job["address"]); ?>" />
                    ...
                    <div>
                        ...
                        <input type="date"  class="calendar_input" gt-isparam="starttime" placeholder="请输入日期：2015-04-01" gt-check="date" value="<?php echo ($job["starttime"]); ?>"/>
                    </div>
                    <div>
                        <span class="label_text">截止时间</span>
                        <input type="hidden" gt-islabel="截止时间"/>
                       <!--  <input type="date" class="calendar_input error " gt-isparam="endtime" placeholder="请输入日期：2015-04-01" gt-check="date" value="<?php echo ($job["endtime"]); ?>"/> -->
                         <input type="date" class="calendar_input" gt-isparam="endtime" placeholder="请输入日期：2015-04-01" gt-check="date" value="<?php echo ($job["endtime"]); ?>"/>
                    </div>
                   <!--  <label for="" class="wrong">截止日期不能大于开始日期</label> -->
                    <div>
                        <span class="label_text">联系人</span>
                        <input type="hidden" gt-islabel="联系人"/>
                        <input type="text"  gt-isparam="contacter" value="<?php echo ($job["contacter"]); ?>"/>
                    </div>
                    <div>
                        <span class="label_text">联系电话</span>
                        <input type="hidden" gt-islabel="联系电话"/>
                        <input type="text" gt-isparam="phone" gt-check="phone" value="<?php echo ($job["phone"]); ?>"/>
                    </div>
                    <!-- <label for="tel" class="wrong">手机号码输入有误</label> -->
                    <div>
                        <span class="label_text">招聘人数</span>
                         <input type="hidden" gt-islabel="招聘人数"/>
                        <input type="text"  gt-isparam="number" gt-check="number" value="<?php echo ($job["number"]); ?>"/>
                    </div>
                    <div>
                        <span class="label_text">工资待遇</span>
                        <input type="hidden" gt-islabel="工资待遇"/>
                        <input type="text" class="width130" gt-isparam="price" value="<?php echo ($job["price"]); ?>"/>
                        <!-- <input type="text" class="width130 error" gt-isparam="price" value="<?php echo ($job["price"]); ?>"/> -->
                        <div class="input-group spinner">
                            <select name="pricetype" gt-isparam="pricetype"  style="width:100%">
                                <option value="<?php echo ($job["pricetype"]); ?>"><?php echo ($job["pricetype"]); ?></option>
                        ...
                        <div class="input-group spinner">
                             <select name="paytype" gt-isparam="paytype"  style="width:100%">
                                    <option value="<?php echo ($job["paytype"]); ?>"><?php echo ($job["paytype"]); ?></option>
                    ...
                    <div style="overflow:hidden;height:auto;">
                        ...
                        <textarea rows="4" cols="30" rows="10" gt-isparam="intro"><?php echo ($job["intro"]); ?></textarea>
                    </div>
                    <input type="button" value="确定发布" class="btn_save" gt-btn-click="confirmModifyJob"/>
                </div>
            </div>
        </div>
        <!-- 个人中心end -->
    ...
    jQuery(document).ready(function () {
        ...
        $("select[name='level1']").on('change', '', function (event) {
            level1 = $(this).val();
             ...
             jQuery.ajax({
                url: CON_PATH + "/getLevel2ListByname",
                type: 'POST',
                data: {'level1': level1},
 ...
 //获取区县列表
    ...
    $("select[name='city']").on('change','',function() {
     ...
     function getCounty() {
        ...
        $.post(MOD_PATH + "/Index/getCountyByCityName", {city: city}, function (data) {
