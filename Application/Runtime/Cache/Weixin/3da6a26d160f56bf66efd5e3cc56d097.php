...
<script>
    ...
    var SEL_PATH = "/weixin.php/Mobile/choseArea";
    var ACT_PATH = "/weixin.php/Mobile/choseArea";
</script>
</head>
<body>
    ...
    <div class="middle row">
        <div class="gtlyout bg-lightgray" style="padding:0">
            <?php if(is_array($firsttypes)): $i = 0; $__LIST__ = $firsttypes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="gtlv-type-title">
                ...
                <!--二级分类-->
                <?php if(is_array($secondtypes)): $i = 0; $__LIST__ = $secondtypes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i; if(($vo["id"]) == $vo2["pid"]): ?><input type="hidden" value="<?php echo ($vo2["name"]); ?>"/>

                        <div class="gtlv-single">
                            <div class="gttv">
                                <a href="pageJobs?area=<?php echo ($vo2["name"]); ?>"><?php echo ($vo2["name"]); ?>
                                </a>
...
<script>
    $(document).ready(function () {
        $(".gtlv-single").on('click', '', function (event) {
            ...
            name = $.trim(name);
            console.log(name);
            location.href = MOD_PATH + "/Jobs/pageJobs?area=" + name;
        });
    });
</script>